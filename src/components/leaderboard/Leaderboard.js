...
export default class Leaderboard extends Component {
  ...
  componentDidMount = async () => {
    try {
      const { data } = await axios.get('https://cluster-duck-server.herokuapp.com/api/users/');
  ...
  displayUsers = () => {
    if (this.state.users !== null && this.state.users[0]) {
      const users = this.state.users
        .slice(0, 10)
        .map((user, rank) => <LeaderboardCard user={user} rank={rank + 1} key={rank + 1} />);
      return users;
    } else return <Spinner style={{ width: '10rem', height: '10rem' }} color="primary" />;
  };
